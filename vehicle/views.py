...
@login_required(login_url='adminlogin')
def issue_loa_view(request,pk):
    mechanic=models.Mechanic.objects.get(id=pk)
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter, bottomup=0)
    # textob2 = c.beginText()
    # textob2.setTextOrigin(inch, inch)
    # textob2.setFont("Courier-Bold", 20)
    ...
    lines = [
        ...
        "",
        "",
        "has worked with Vehicle Repair Hub since 2019.",
        "",
        "",
        "During his tenure, we found him very active and competent in executing all",
        "assigned tasks and services in a very efficient manner.",
        "",
        "We wish him great success in all of his future endeavours.",
        "",
        "",
        "",
        "With Regards",
        ...
        "",
        "",
        "                    This letter has been digitally generated by VRH Pvt Ltd",
    ]

    # for pine in pines:
    #     textob.textLine(pine)
    ...
    # c.drawText(textob2)
    c.drawText(textob)
    c.drawImage(logo,-5*inch,-1.75*inch) 
    c.showPage()
    c.save()
    buf.seek(0)

    return FileResponse(buf, as_attachment=True, filename='loa.pdf')
...
def download_invoice_view(request,pk):
    customer=models.Customer.objects.get(user_id=request.user.id)
    enquiries=models.Request.objects.all().filter(customer_id=customer.id).exclude(status='Pending')
    current=models.Request.objects.get(id=pk)
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter, bottomup=0)
    textob = c.beginText()
    textob.setTextOrigin(inch, inch)
    ...
    lines = [
        ...
        "",
        "Customer Info",
        "Client Name: "+customer.get_name,
        "Address: "+customer.address,
        "Mobile: "+customer.mobile,
        "",
        "Vehice Info",
        "Make & Model: "+current.vehicle_brand + " " + current.vehicle_name + " " + current.vehicle_model,
        "Registration No: "+current.vehicle_no,
        "",
        "Mechanic Name: "+current.mechanic.get_name,
        "",
        "",

        "Job Description: " + current.problem_description,
    ]
    ...
    data=[('Total','',current.cost),('5.','',''),('4.','',''),('3.','Labour','1000'),('2.','Airbag Unit','2500'),('1.','Steering Column','6500'),('S.no.','Particulars','Amount')]
    width = 150
    height = 30
    x = 80
    y = 400
    table = Table(data,width,height)
    table.setStyle([
